...
public static class Utility
{
    ...
    public static string GetLogFilePath(ScriptData scriptData)
    {
        string pathDir = string.Format("{0}/LogFile/{1}/Log", Application.dataPath, scriptData.uuid);
        if (!Directory.Exists(pathDir))
        {
            Debug.Log("创建文件夹" + pathDir);
            Directory.CreateDirectory(pathDir);
        }
        string filePath = pathDir + "/" + DateTime.Now.ToString("yyyy-MM-dd") + ".txt";
        if (!File.Exists(filePath))
        {
            Debug.Log("创建文件" + filePath);
            File.CreateText(filePath).Dispose();
        }
        return filePath;
    }

    public static string GetImgDirPath(ScriptData scriptData)
    {
        string pathDir = string.Format("{0}/LogFile/{1}/Image/{2}", Application.dataPath, scriptData.uuid, DateTime.Now.ToString("yyyy-MM-dd"));
        if (!Directory.Exists(pathDir))
        {
            Debug.Log("创建文件夹" + pathDir);
            Directory.CreateDirectory(pathDir);
